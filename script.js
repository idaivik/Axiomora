...
if (typeof gsap !== 'undefined') {
    ...
    gsap.to(".task-progress-arc", {
        scrollTrigger: { trigger: ".widget-task-completion", start: "top 85%" },
...
const planSelectLinks = document.querySelectorAll('[data-select-plan]');
...
if (planSelectLinks.length > 0) {
    planSelectLinks.forEach((link) => {
        link.addEventListener('click', (event) => {
            const plan = link.dataset.selectPlan;
            ...
            link.href = `payment.html?${params.toString()}`;
        });
    });
}
